...
mod tests {
    ...
    #[test]
    fn clear_requests_have_no_reset_knob_and_default_to_refusing() {
        // Parity audit P0.1: ISTA's whole-vehicle clear sends no UDS 0x11, so
        // klartext's clear has no reset step and therefore no flag to control one.
        // A stale client still passing `reset` must not silently re-enable
        // anything — serde ignores the unknown field, and the request still
        // carries only `confirm`.
        let stale: ClearAllFaultsRequest =
            serde_json::from_str(r#"{"confirm":true,"reset":true}"#).unwrap();
        assert!(stale.confirm);

        // `confirm` itself still defaults to refusing — the gate that matters.
        let bare: ClearFaultsRequest = serde_json::from_str(r#"{"ecu":"0x12"}"#).unwrap();
        assert!(!bare.confirm);
        let bare_all: ClearAllFaultsRequest = serde_json::from_str("{}").unwrap();
        assert!(!bare_all.confirm);
    }
}
